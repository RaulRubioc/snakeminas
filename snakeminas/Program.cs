...
namespace SnakeGame
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            DrawFood();
            DrawSnake();
            PlaceMines(); // Coloca minas en el juego
            ...
            while (!gameOver)
            {
                ...
                if (IsHittingMine()) // Verifica si la serpiente golpea una mina
                {
                    gameOver = true;
                    break;
                }
        ...
        {
            for (int i = 0; i < 5; i++) // Coloca 5 minas
            {
                int[] mine = new int[2] { random.Next(1, width), random.Next(1, height) };
                mines.Add(mine);
